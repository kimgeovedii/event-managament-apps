...
interface UserOrderDetailViewClientProps {
  initialOrder: Order;
}

export default function UserOrderDetailViewClient({ initialOrder }: UserOrderDetailViewClientProps) {
  const router = useRouter();
  const { data: order } = useGetOrder(initialOrder.id.toString(), initialOrder);
  ...
  return (
    <div className="max-w-6xl mx-auto p-3 md:p-8">
      ...
      <OrderDetail order={order || initialOrder} />
    </div>
  );
}
